...
const FuncionObj = (props) => {
    ...
    .map((vari, index) => (
      <React.Fragment key={"divIF" + index}>
        ...
        <InputGroupAddon key={"ADDIF" + index} addonType="append">
          <InputGroupText key={"IFD" + index}>
            {"X" + (vari.xi + 1)}
          </InputGroupText>
        </InputGroupAddon>
        {index <
          variables.filter((vari) => vari.descripcion !== "").length - 1 && (
          <InputGroupText key={"+IF" + index}>+</InputGroupText>
        )}
      </React.Fragment>
    ));
  ...
  return (
    <InputGroup key={"INPUTFUNCIONAL"}>
      {variablesToFunction}
      <InputGroupText className="mx-1" key="IFOPT">
        {" => " + props.objective.toUpperCase()}
      </InputGroupText>
    </InputGroup>
  );
};
