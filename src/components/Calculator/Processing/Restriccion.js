...
const Restriccion = (props) => {
  ...
  let botoneraMaxMin = (
    <ButtonGroup className="mx-1" key={"Eq" + props.restriccion.ri}>
      <Button
        key={"B<s" + props.restriccion.ri}
        name="eq"
        onClick={(e) => {
          props.handleCoefRes(e, props.restriccion.ri);
          thisEq = "<=";
        }}
  ...
  let inputsRestriccions = coeficientes.map((coeficiente, indx) => (
    <React.Fragment key={"divCi" + indx}>
      <Input
        key={"Ci" + props.restriccion.ri + "r" + indx}
        type="number"
        name={indx}
      ...
      />
      <InputGroupAddon
        key={"IADD" + props.restriccion.ri + "r" + indx}
        addonType="append"
      >
        <InputGroupText key={"r" + props.restriccion.ri + "r" + indx}>
          {"X" + (indx + 1)}
        </InputGroupText>
      </InputGroupAddon>
      {indx < coeficientes.length - 1 && (
        <InputGroupText key={"+" + props.restriccion.ri + "r" + indx}>
          +
        </InputGroupText>
      )}
    </React.Fragment>
  ...
  return (
    <Card key={"CR" + props.restriccion.ri} id={"CR" + props.restriccion.ri}>
      <UncontrolledTooltip
        flip={false}
        key={"TTCR" + props.restriccion.ri}
        placement="auto"
        target={"CR" + props.restriccion.ri}
      >
        {"Aqui debes ingresar los coeficiente de " +
          props.restriccion.descripcion}
      </UncontrolledTooltip>
      <CardHeader className="p-0 m-0">
        <CardTitle>
          {"R" +
            (props.restriccion.ri + 1) +
            ":" +
            props.restriccion.descripcion}
        </CardTitle>
      </CardHeader>
      <CardBody className="p-0 my-1 mx-auto">
        <InputGroup key={"RT" + props.restriccion.ri}>
          ...
          {botoneraMaxMin}
          <Input
            key={"C" + props.restriccion.ri + "r" + coeficientes.length}
            type="number"
            name={"derecha"}
            placeholder="Coe"
            className="InputCoeficiente"
