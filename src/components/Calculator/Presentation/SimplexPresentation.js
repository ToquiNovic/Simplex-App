...
class SimplexPresentation extends React.Component {
  ...
  cardsVariablesRender = (variables, result) =>
    variables
      ...
      .map((vari) => (
        ...
        >
          <CardHeader>
            <CardTitle>{"Variable: X" + (vari.xi + 1)}</CardTitle>
          </CardHeader>
          <CardBody>
            <Row>
              <CardText>
          ...
          <CardFooter>
            ...
            {result.solutionSet[vari.xi]
              ? this.tablaDeRecursosFoot(result.solutionSet[vari.xi], vari.xi)
  ...
  render() {
    ...
    let elementosTabAnalisis = itemsTabAnalisis.map((item, index) => (
      <tr key={"T-A-" + index}>
        <td>{item.name}</td>
        <td>{item.item}</td>
        <td>{item.value}</td>
      ...
      <Card outline color="secondary" className="w-100 mt-3 mx-auto">
        ...
        <CardBody>
          <Table>
            ...
            <tbody>{elementosTabAnalisis}</tbody>
          </Table>
        </CardBody>
      </Card>
    );
        ...
        <CardHeader>
          <Row>
            <Col className="text-left">
              <CardTitle>
                <h5>Detalle de Variables y Recursos:</h5>
              </CardTitle>
            </Col>
            <Col>
              <Button
                ...
                color={!this.state.details ? "success" : "danger"}
              >
                {!this.state.details ? "Ver Detalles" : "Ocultar Detalles"}
              </Button>
            </Col>
          </Row>
        </CardHeader>
        <Collapse isOpen={this.state.details}>
          <CardBody>{this.cardsVariablesRender(variables, result)}</CardBody>
        </Collapse>
      </Card>
    );
    return (
