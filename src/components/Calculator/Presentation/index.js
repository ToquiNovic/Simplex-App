...
let convertAppToModelForSolverPrimal = (datosApp) => {
  ...
  variables.forEach((vari) => {
    ...
    restricciones.forEach(
      (restri) => (newVari["r" + restri.ri] = restri.coeficientes[vari.xi])
    );
    // console.log(newVari);
...
class Presentation extends React.Component {
  ...
  validateCoeficientes = (props) => {
    ...
    let verifQty = varsOperatives.length
      ? varsOperatives.every((va) => va.coeficiente !== "")
      : false;
    let restOperatives = restricciones.filter((re) => re.descripcion !== "");
    let veriResQty = restOperatives.length
      ? restOperatives.every(
  ...
  render() {
    ...
    return (
      <>
        <Card color="success" className="w-120 mt-4 mx-auto">
          <CardHeader>
            <CardTitle>
                ...
                <Row className="justify-content-center">
                  <h4>
                    <Badge pill color="info">
                      Existen soluciones multiples para este ejercicio
                    </Badge>
                  </h4>
                </Row>
              )}
