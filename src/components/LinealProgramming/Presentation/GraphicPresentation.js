...
class GraphicPresentation extends React.Component {
  ...
  updateState = () => {
    ...
    let { lines, expresiones, highestValueX, highestValueY } =
      this.getLinesAndExpressions(restricciones);
    //Obtenemos los Puntos de marca general
    let { points, convexPoints } = this.getPoints(
      restricciones,
      expresiones,
      result,
      highestValueX,
  ...
  getLinesAndExpressions = (restricciones) => {
    const getFrac = (real) =>
      new Fraction(
        Math.pow(10, (real - real.toFixed()).toString().length - 2) * real,
        Math.pow(10, (real - real.toFixed()).toString().length - 2)
      );
    //Tipos de Expresiones: 0: Constante en X; 1: Constante en Y; 2: Recta con pendiente.
    let expresiones = [];
    let arrayDeRestriccionesConLosDosCoef = restricciones.filter(
      (el) => el.coeficientes[0] > 0 && el.coeficientes[1] > 0
    );
    let highestValueY = Math.max.apply(
      Math,
      arrayDeRestriccionesConLosDosCoef.map(
        (restri) => restri.derecha / restri.coeficientes[1]
      )
    );
    let highestValueX = Math.max.apply(
      Math,
      arrayDeRestriccionesConLosDosCoef.map(
        (restri) => restri.derecha / restri.coeficientes[0]
      )
    );
    // console.log('Ymax: '+highestValueY+' Xmax:'+highestValueX);

    let lines = restricciones.map((restri) => {
      let xNum = !Number.isInteger(Number(restri.coeficientes[0]))
        ? getFrac(Number(restri.coeficientes[0]))
        : Number(restri.coeficientes[0]);

      let yNum = !Number.isInteger(Number(restri.coeficientes[1]))
        ? getFrac(Number(restri.coeficientes[1]))
        : Number(restri.coeficientes[1]);
      ...
      if (xNum !== 0 && yNum !== 0) {
        ...
        if (xEqu >= 0 && yEqu >= 0) {
          ...
          if (restri.eq === ">=") {
            return [
              { x: 0, y: yEqu, y0: highestValueY },
              { x: xEqu, y: 0, y0: highestValueY },
              { x: highestValueX, y: 0, y0: highestValueY },
            ];
          } else {
            return [
              { x: 0, y: yEqu },
  ...
  getColorList = (restricciones) =>
    restricciones.map((restri) =>
      Object({
        title: "R" + (restri.ri + 1) + " Tipo:" + restri.eq,
        color: randomColor({ hue: "random", luminosity: "ligth" }),
      })
    );
  ...
  getOptimPoint = (solSet) => {
    ...
    if (solSet["1"] && solSet["2"]) {
      return {
        x: Number(solSet["1"]).toFixed(2),
        y: Number(solSet["2"]).toFixed(2),
        P: "1 - OPTIMO",
      };
    } else if (solSet["1"]) {
      return {
    ...
    if (optimPoint) {
      try {
        if (variables[0].coeficiente !== 0 && variables[1].coeficiente !== 0) {
          let xPoint = !Number.isInteger(Number(optimPoint.x))
            ? getFrac(Number(optimPoint.x))
            : Number(optimPoint.x);

          let yPoint = !Number.isInteger(Number(optimPoint.y))
            ? getFrac(Number(optimPoint.y))
            : Number(optimPoint.y);

          let xExp = new Expression("x")
            .subtract(xPoint)
            .multiply(variables[0].coeficiente);
          let yExp = new Expression("y")
            .subtract(yPoint)
            .multiply(variables[1].coeficiente);
  ...
  getPoints = (restricciones, expresiones, solSet, xMax, yMax) => {
    ...
    //   );
      ...
      while (pointsList.length) {
        ...
        let minAngle = pointsList.reduce(
          (min, p) => (calcAng(point, p) < min ? calcAng(point, p) : min),
          361
        );
        if (minAngle < 361) {
          let indNewPoint = pointsList.findIndex(
            (p) => calcAng(point, p) === minAngle
          );
    ...
    const verifyPoint = (point, restricciones, points) => {
      if (point.x >= 0 && point.y >= 0) {
        if (!verifyPointInPoints(point, points)) {
          if (verifyPointInRestrictions(point, restricciones)) {
            return true;
          } else return false;
        } else return false;
      } else return false;
    ...
    const verifyPointInPoints = (point, points) =>
      points.some(
        (pointL) =>
          pointL.x === point.x.toFixed(2) && pointL.y === point.y.toFixed(2)
      );

    //Funcion que se encarga de verificar que un punto cumpla con todas las Restricciones.
    const verifyPointInRestrictions = (point, restricciones) =>
      restricciones.every((restri) => {
        let calIzq =
          restri.coeficientes[0] * point.x + restri.coeficientes[1] * point.y;
        if (restri.eq === ">=") {
    ...
    const getPointFromExpCExpY = (expC, expY) => {
      ...
      let expResultY = Number(expY.solveFor("y"));
      let expResultX = Number(
        new Equation(expC.solveFor("y"), expY.solveFor("y")).solveFor("x")
      );
      //Verificamos que no se corten en algun otro cuadrante que no sea el de analisis.
    ...
    const getPointFromExpCExpX = (expC, expX) => {
      ...
      let expResultX = Number(expX.solveFor("x"));
      let expResultY = Number(
        new Equation(expC.solveFor("x"), expX.solveFor("x")).solveFor("y")
      );
      //Verificamos que no se corten en algun otro cuadrante que no sea el de analisis.
      if (expResultX >= 0 && expResultY >= 0) {
        //Generamos el Punto.
    ...
    const getPointFromTwoExpC = (exp1, exp2) => {
      let expResultX = Number(
        new Equation(
          exp1.restriEquation.solveFor("y"),
          exp2.restriEquation.solveFor("y")
        ).solveFor("x")
      );
      let expResultY = Number(
        new Equation(
          exp1.restriEquation.solveFor("x"),
          exp2.restriEquation.solveFor("x")
        ).solveFor("y")
      );
      //Verificamos que no se corten en algun otro cuadrante que no sea el de analisis.
      if (expResultX >= 0 && expResultY >= 0) {
    ...
    const getPointFromTwoExp = (exp1, exp2) => {
      try {
        ...
        if (exp1.tipo === 2 && exp2.tipo === 2) {
          ...
          if (exp2.tipo === 0) {
            return getPointFromExpCExpX(
              exp1.restriEquation,
              exp2.restriEquation
            );
          } else
            return getPointFromExpCExpY(
              exp1.restriEquation,
              exp2.restriEquation
            );
        } else if (exp2.tipo === 2) {
    ...
    // if (Object.entries(solSet).length) {
  ...
  getTableResult = (points, coeficientes, restricciones) => {
    const calcSlacksValue = (point) => {
      return restricciones.map((restri) => (
        <td key={"S-C-" + point.P + "-" + restri.ri}>
          {Math.abs(
            restri.coeficientes[0] * point.x +
              restri.coeficientes[1] * point.y -
              restri.derecha
          ).toFixed(2)}
        </td>
      ));
    ...
    const calcResult = (point) => {
      return Math.abs(
        coeficientes.x * point.x + coeficientes.y * point.y
      ).toFixed(2);
    };
    // console.log("IMPRIMIENDO RESULT:" + calcResult);
    let slacksTitles = restricciones.map((restri) => (
      <th key={"S-T-" + (restri.ri + 1)}>{"S" + (restri.ri + 1)}</th>
    ));
    return (
      <Table>
        <thead>
        ...
        <tbody>
          {points.map((point) => (
            <tr key={"T-P-" + point.P}>
              <td>P:{point.P + 1}</td>
              <td>{calcResult(point)}</td>
              <td>{point.x}</td>
              <td>{point.y}</td>
              {calcSlacksValue(point)}
            </tr>
          ))}
        </tbody>
      </Table>
  ...
  mapperAreaSeries = (lines, referencias) =>
    lines.map((data, index) => (
      <AreaSeries
        key={"L-S-A" + index}
        opacity={0.3}
        color={referencias.length > 0 ? referencias[index].color : "red"}
        data={data}
      />
    ));
    ...
    return (
      <CardBody>
        <Card outline color="secondary">
            ...
            <Row>
              ...
              <Col>
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    this.setState({ areaGraph: !this.state.areaGraph })
                  }
                  color={!this.state.areaGraph ? "success" : "danger"}
                >
                  {!this.state.areaGraph
                    ? "Ver Sombra de Restricciones"
                    : "Ocultar Sombra de Restricciones"}
                </Button>
              </Col>
            </Row>
          </CardHeader>
          ...
          <CardFooter>{tableResult}</CardFooter>
        </Card>
      </CardBody>
    );
  }
